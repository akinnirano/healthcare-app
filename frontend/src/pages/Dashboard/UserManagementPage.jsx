...
export default function AdminDashboard() {
  ...
  const [active, setActive] = useState("user_mgmt"); // default to Manage Users
  const [mobileOpen, setMobileOpen] = useState(false);
  const [stats, setStats] = useState({ users: 0, patients: 0, staff: 0, assignments: 0 })
    ...
    async function loadCounts(){
      try {
        const [u, p, s, a] = await Promise.all([
          api.get('/users', { params: { limit: 1_000 } }),
          api.get('/patients', { params: { limit: 1_000 } }),
          api.get('/staff', { params: { limit: 1_000 } }),
          api.get('/assignments', { params: { limit: 1_000 } }),
        ])
        setStats({
        ...
        <div className="w-full px-4 sm:px-6 py-6 grid grid-cols-12 gap-6">
          ...
          <main className="col-span-12 md:col-span-9 lg:col-span-10 xl:col-span-10 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
              <StatCard title="Total Users" value={String(stats.users)} badge="All" />
              <StatCard title="Total Patients" value={String(stats.patients)} badge="Registered" />
              <StatCard title="Total Staff" value={String(stats.staff)} badge="Active" />
              <StatCard title="Assignments" value={String(stats.assignments)} badge="All" />
            </div>

            {/* Manage Users block placed where charts would be */}
            <div className="rounded-2xl border border-slate-100 bg-white p-4 md:p-6 shadow-sm">
              <h2 className="text-lg font-semibold text-slate-800 mb-2">Manage Users</h2>
              <ManageUsers />
            </div>

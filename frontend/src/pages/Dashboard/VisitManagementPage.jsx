...
export default function VisitManagementPage() {
  ...
  const [form, setForm] = useState({ patient_id: "", staff_id: "", scheduled_time: "", notes: "" })
        ...
        <div className="w-full px-4 sm:px-6 py-6 grid grid-cols-12 gap-6">
          ...
          <main className="col-span-12 md:col-span-9 lg:col-span-10 xl:col-span-10 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
              <StatCard title="Total Visits" value={String(stats.visits)} badge="All" />
            </div>

            {/* Create Visit */}
            <div className="rounded-2xl border border-slate-100 bg-white p-4 md:p-6 shadow-sm">
              <h2 className="text-lg font-semibold text-slate-800 mb-2">Create Visit</h2>
              ...
              <form onSubmit={async (e)=>{
                ...
                try {
                  const iso = form.scheduled_time ? new Date(form.scheduled_time).toISOString() : null
                  const payload = { patient_id: form.patient_id ? parseInt(form.patient_id, 10) : null, staff_id: form.staff_id ? parseInt(form.staff_id, 10) : null, scheduled_time: iso, notes: form.notes || null }
                  await api.post('/visits', {}, { params: payload })
                  setMessage('Visit created successfully');
                  setForm({ patient_id: '', staff_id: '', scheduled_time: '', notes: '' })
                } catch (err){
                ...
                <label className="block">
                  <span className="block text-sm text-slate-600">Patient</span>
                  <select value={form.patient_id} onChange={(e)=>setForm(f=>({...f, patient_id: e.target.value}))} required className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2">
                ...
                <label className="block">
                  ...
                  <select value={form.staff_id} onChange={(e)=>setForm(f=>({...f, staff_id: e.target.value}))} className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2">
                    <option value="">Select staff</option>
                    {staff.map(s => (<option key={s.id} value={s.id}>{s.full_name || s.name || `Staff #${s.id}`}</option>))}
                  </select>
                </label>
                <label className="block">
                  <span className="block text-sm text-slate-600">Scheduled Time</span>
                  <input type="datetime-local" value={form.scheduled_time} onChange={(e)=>setForm(f=>({...f, scheduled_time: e.target.value}))} required className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2" />
                </label>
                <label className="block md:col-span-2">
                  <span className="block text-sm text-slate-600">Notes</span>
                  <input value={form.notes} onChange={(e)=>setForm(f=>({...f, notes: e.target.value}))} className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2" />
                </label>
                <div className="md:col-span-2">
                  <button className="mt-2 rounded-lg bg-sky-600 px-5 py-2 text-white shadow hover:bg-sky-700">Create Visit</button>
                </div>
              </form>
            </div>
